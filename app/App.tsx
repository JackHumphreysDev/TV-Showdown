...
export default function App() {
  ...
  const [editingItem, setEditingItem] = useState<Item | null>(null), [editTitle, setEditTitle] = useState(''), [editKind, setEditKind] = useState<Kind>('movie'), [editYear, setEditYear] = useState(''), [editNote, setEditNote] = useState('');
  ...
  const removeItem = (item: Item) => {
    const remove = () => perform(async () => { await api(`/api/watchlist/${item.id}`, token, 'DELETE'); setWatchlist(await api('/api/watchlist', token)); if (editingItem?.id === item.id) setEditingItem(null); if (groupId) await refreshGroup(token!, groupId); });
  ...
  const editItem = (item: Item) => { setEditingItem(item); setEditTitle(item.title); setEditKind(item.kind); setEditYear(item.year ? String(item.year) : ''); setEditNote(item.note || ''); };
  const saveItem = () => perform(async () => {
    if (!editingItem) return;
    await api(`/api/watchlist/${editingItem.id}`, token, 'PATCH', { note: editNote, ...(!editingItem.tmdbId ? { title: editTitle, kind: editKind, year: editYear } : {}) });
    setWatchlist(await api('/api/watchlist', token)); setEditingItem(null); if (groupId) await refreshGroup(token!, groupId);
  });
  ...
  return <View style={s.root}><StatusBar barStyle="light-content" /><View style={[s.shell, desktop && s.desktopShell]}>
    ...
    <View style={s.main}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[s.content, !desktop && s.contentMobile]}>
      ...
      {page === 'watchlist' && <><Text style={s.eyebrow}>YOUR PICKS</Text><Text style={s.title}>Watchlist</Text><Text style={s.muted}>Only you can edit your list. Eligible titles are visible to your groups.</Text>
        ...
        </View>{editingItem && <View style={s.card}><Text style={s.eyebrow}>EDIT WATCHLIST ITEM</Text>{editingItem.tmdbId ? <><Text style={s.section}>{editingItem.title}</Text><Text style={s.small}>Catalogue title details stay linked to their source. You can still update your private note.</Text></> : <><Field label="Title" value={editTitle} onChangeText={setEditTitle} /><View style={s.row}><Chip label="Film" active={editKind === 'movie'} onPress={() => setEditKind('movie')} /><Chip label="Series" active={editKind === 'series'} onPress={() => setEditKind('series')} /></View><Field label="Release year (optional)" value={editYear} onChangeText={setEditYear} placeholder="2024" /></>}<Field label="Private note (optional)" value={editNote} onChangeText={setEditNote} placeholder="Why you saved it, who recommended it…" multiline /><Text style={s.small}>Only you can see this note. It is never shared with your groups.</Text><View style={s.row}><Button label="Save changes" onPress={saveItem} disabled={busy || (!editingItem.tmdbId && (!editTitle.trim() || (!!editYear && !/^\d{4}$/.test(editYear))))} /><Button label="Cancel" quiet onPress={() => setEditingItem(null)} /></View></View>}<Text style={s.section}>Your titles <Text style={s.small}>· {watchlist.length} total, {eligibleOwn} eligible</Text></Text>
        {!watchlist.length ? <Text style={s.muted}>Your list starts here. Add a film or series above.</Text> : watchlist.map((item) => <View key={item.id} style={s.itemCard}><Poster title={item.title} path={item.posterPath} size={62} /><View style={{ flex: 1, gap: 6 }}><Text style={s.white}>{item.title}</Text><Text style={s.small}>{kindLabel(item.kind)}{item.year ? ` · ${item.year}` : ''}</Text>{item.note ? <Text style={s.privateNote} numberOfLines={2}>Private note · {item.note}</Text> : null}<View style={s.row}>{(['want', 'watching', 'watched'] as Status[]).map((status) => <Chip key={status} label={statusLabel(status)} active={item.status === status} onPress={() => changeStatus(item, status)} />)}</View><View style={s.row}><Pressable accessibilityRole="button" onPress={() => editItem(item)}><Text style={s.goldText}>Edit</Text></Pressable><Pressable accessibilityRole="button" onPress={() => removeItem(item)}><Text style={s.remove}>Remove</Text></Pressable></View></View></View>)}
      </>}
...
const s = StyleSheet.create({
  ...
  offer: { flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#48424A', paddingVertical: 10, gap: 10 }, itemCard: { backgroundColor: '#1E1D21', borderColor: '#39353C', borderWidth: 1, borderRadius: 15, padding: 13, flexDirection: 'row', alignItems: 'center', gap: 13 }, listRow: { flexDirection: 'row', alignItems: 'center', gap: 12, borderBottomWidth: 1, borderBottomColor: '#3C3740', paddingVertical: 9 }, privateNote: { color: '#C7B58E', fontSize: 12, lineHeight: 17 }, remove: { color: '#DB999D', fontSize: 13, marginTop: 3 }, preview: { backgroundColor: '#363029', borderRadius: 12, padding: 13, gap: 10 }, code: { color: gold, fontSize: 28, fontWeight: '900', letterSpacing: 4 }, memberLine: { flexDirection: 'row', alignItems: 'center', gap: 11 }, historyStar: { color: gold, backgroundColor: '#463621', fontSize: 23, width: 44, height: 44, borderRadius: 22, textAlign: 'center', lineHeight: 44 },
